...
.. note::
    ...
    manually specify the widths.

.. note::
    You may want to add this a documentation dependency to your setup procedure.
    As any other semver-following project, it is suggested to pin the exact version you
    are using for reproducibility and not breaking the build process once there are
    ...
    ``sphinx-datatables~=x.y.z``. The latter can only match ``x.y.a / a >= z``
    stable releases, meaning only patches will make it into your doc build process.
